...
TOOLS = [
    ...
    tool('input', 'Act with frame + image-pixel coordinates (or generation + global pixels). Types: move, click (count 1-3), drag (to_x/to_y or path), scroll (dx/dy steps, optional x/y), type (Unicode), key (["CTRL","L"]), key_down/key_up, mouse_down/mouse_up. modifiers holds keys around an action. screenshot=true observes afterwards. window is the address (from windows) of the window you are typing into: REQUIRED for type/key/key_down/key_up, and checked before every keystroke so input fails rather than landing somewhere else if focus moves.',
